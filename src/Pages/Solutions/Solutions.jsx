...
import dataMigrationImg from "../../assets/dataMigration.jpeg";
import tailoredBusinessSolutionImg from "../../assets/tailoredBusinessSolution.png";
...
const Solutions = () => {
  return (
    <Layout>
      {/* Hero Section */}
      <div className="relative bg-gradient-to-br from-indigo-50 to-blue-50 py-8 md:py-16 px-4 md:px-12 ">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <div className="space-y-6 text-center md:text-left animate-fade-in">
            <h1 className="text-3xl md:text-4xl lg:text-6xl font-bold text-gray-900 leading-tight ">
              Tailored{" "}
              <span className="text-indigo-600">Business Solutions</span>
            </h1>
            <p className="text-gray-600 text-base md:text-lg max-w-xl mx-auto md:mx-0 leading-relaxed ">
              Unlock your business potential with our customized solutions
          ...
          <div className="relative flex justify-center md:justify-end">
            <img
              src={tailoredBusinessSolutionImg}
      ...
      {/* Services Section */}
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 space-y-12">
        {data.map((item, index) => (
          <ServiceCard
            key={index}
            src={item.src}
        ...
        <div className="max-w-8xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          >
            ...
            <p className="text-gray-600 text-base md:text-xl leading-relaxed">
              Our data migration tools ensure smooth transitions between
              systems, transforming and preparing your data for compatibility
              with new environments.
            </p>

            <ul className="grid grid-cols-1 sm:grid-cols-2 place-items-center md:place-items-start gap-4 my-6">
              {importList.map(({ label, icon: Icon }, index) => (
                <motion.li
                  key={label}
                  className="flex items-center gap-3 text-gray-700 text-base font-medium hover:text-indigo-600 transition-colors duration-300"
                  initial={{ opacity: 0, x: index % 2 === 0 ? -150 : 150 }} // slide from left/right
                  whileInView={{ opacity: 1, x: 0 }}
                  transition={{
                    duration: 0.6,
                    delay: index * 0.15, // stagger items
                    ease: "easeOut",
                  }}
                  viewport={{ once: false, amount: 0.2 }}
                >
